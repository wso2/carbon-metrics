...
public class MetricServiceTest extends TestCase {
    ...
    public void testMetricSetLevel() {
        ...
        String identifier = "test-metric-level";
        Meter meter = MetricManager.meter(Level.INFO, name, identifier);
        assertNull("There should no configured level", metricService.getMetricLevel(name, identifier));

        metricService.setRootLevel(Level.TRACE);
        meter.mark();
        assertEquals("Count should be one", 1, meter.getCount());

        metricService.setMetricLevel(name, identifier, Level.INFO);
        assertEquals("Configured level should be INFO", Level.INFO, metricService.getMetricLevel(name, identifier));
        meter.mark();
        assertEquals("Count should be two", 2, meter.getCount());

        metricService.setMetricLevel(name, identifier, Level.DEBUG);
        assertEquals("Configured level should be DEBUG", Level.DEBUG, metricService.getMetricLevel(name, identifier));
        meter.mark();
        ...
        metricService.setMetricLevel(name, identifier, Level.ALL);
        assertEquals("Configured level should be ALL", Level.ALL, metricService.getMetricLevel(name, identifier));
        meter.mark();
        assertEquals("Count should be five", 5, meter.getCount());

        metricService.setMetricLevel(name, identifier, Level.OFF);
        assertEquals("Configured level should be OFF", Level.OFF, metricService.getMetricLevel(name, identifier));
        meter.mark();
        assertEquals("Count should be five", 5, meter.getCount());
