...
import org.wso2.carbon.metrics.api.ReporterService;
...
/**
 * Description about ReporterDAOTest
 */
public class ReporterDAOTest {
...
//    private final Clock clock = mock(Clock.class);
    ...
    private static final String SOURCE = ReporterDAOTest.class.getSimpleName();
...
//        when(clock.getTime()).thenReturn(System.currentTimeMillis());
        ...
        this.reporter = JDBCReporter.forRegistry(registry).convertRatesTo(TimeUnit.SECONDS)
                .convertDurationsTo(TimeUnit.NANOSECONDS).filter(MetricFilter.ALL)
        ...
        reportCounterValues();
        reportMeterValues();
        reportHistogramValues();
        reportTimerValues();
    }

    @SuppressWarnings("rawtypes")
    private void reportGaugeValues() {
        final Gauge gauge = mock(Gauge.class);
        when(gauge.getValue()).thenReturn(1);
        reporter.report(map("test.gauge", gauge), this.<Counter> map(), this.<Histogram> map(), this.<Meter> map(),
                this.<Timer> map());
    }

    @SuppressWarnings("rawtypes")
    private void reportCounterValues() throws Exception {
    ...
    @SuppressWarnings("rawtypes")
    private void reportHistogramValues() throws Exception {
    ...
    @SuppressWarnings("rawtypes")
    private void reportMeterValues() throws Exception {
    ...
    @SuppressWarnings("rawtypes")
    private void reportTimerValues() throws Exception {
    ...
    public void testQuery() {
        ReporterDAO reporterDAO = new ReporterDAO(dataSource);
        ReporterService reporterService = new ReporterService();
        reporterService.setReporterDAO(reporterDAO);
        // reporterService.metricsFind("*");

//        reporterDAO.queryMetricDatapoints(MetricType.TIMER, "test.timer", MetricAttribute.P99,
//                System.currentTimeMillis() / 1000 - 30, System.currentTimeMillis() / 1000, 10000);
        // System.out.println(reporterDAO.querySources());
    }
}
